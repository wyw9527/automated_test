...
{                                                              
	switch(getSn_SR(SOCK_UDPS))                                                /*获取socket的状态*/
	{
		...
		case SOCK_UDP:                                                           /*socket初始化完成*/
			...
			{
				UDPbuff[len-8]=0x00; 																								 /*添加字符串结束符*/
				recvfrom(SOCK_UDPS,UDPbuff, len, remote_ip,&UDP_port);            /*W5500接收计算机发送来的数据*/                                                   
				printf("%s\r\n",UDPbuff);                                            /*打印接收缓存*/ 
				sendto(SOCK_UDPS,UDPbuff,len-8, remote_ip, UDP_port);             /*W5500把接收到的数据发送给Remote*/
			}
			break;	
	}
}
